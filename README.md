...
- **강제 스캔 OCR 모드 (F9)**: 바코드가 없는 텍스트 라벨용.
  - SNAPI 규격상 "디코드 실패"가 PC로 통지되지 않아 트리거 2회 감지는 불가 → 대신 모드를 켜면 **트리거 1회 = 촬영**이 됩니다.
  - 촬영 이미지에서 ① 소프트웨어 바코드 디코드(ZXing) 시도 → ② 없으면 **강제 OCR 규칙**으로 값 추출.
  - 기본 규칙(설정에서 수정 가능):
